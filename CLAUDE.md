...
- **WAV format**: 16-bit PCM (`bits_per_sample=16`, `int16_t` samples). INMP441 outputs 24-bit I2S data MSB-aligned in a 32-bit DMA word. `process_sample` extrait les 24 bits (`>> 8`), les filtre en float, retourne un `int32_t` 24-bit. `write_buffer` convertit en 16-bit (`>> 8` supplémentaire) avant l'écriture FatFS.
